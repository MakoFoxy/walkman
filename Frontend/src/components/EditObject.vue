                ...
                <v-flex xs12 sm12 md12>
                    <v-card>
                        <v-toolbar dark color="primary">
                            <v-toolbar-title>Редактирование объекта</v-toolbar-title>
                            <v-spacer></v-spacer>
                        </v-toolbar>
                        <v-card-text>
                            <v-form ref="form">
                                <v-container>
                                    <v-layout>
                                        <v-flex>
                                            <v-combobox :items="cities" v-model="object.city" return-object item-value="id" item-text="name" label="Выберите город"></v-combobox>
                                        </v-flex>
                                        <v-flex>
                                            <v-text-field name="name" label="Название объекта" type="text" v-model.trim="object.name" :rules="requiredRule"></v-text-field>
                                        </v-flex>
                                        <v-flex>
                                            <v-text-field name="priority" label="Колонка при добавлении рекламы, например: 2" type="number" v-model="object.priority" :rules="priorityRule"></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                </v-container>
                                <v-container>
                                    <v-layout>
                                        <v-flex xs12 md6>
                                            <v-text-field name="actual-address" label="Фактический адрес" type="text"
                                                v-model.trim="object.actualAddress" :rules="requiredRule"></v-text-field>
                                        </v-flex>
                                        <v-flex xs12 md6>
                                            <v-text-field name="bin" label="БИН" type="text" v-model.trim="object.bin" :rules="requiredRule"></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                </v-container>
                                <v-container>
                                ...
                                <v-container>
                                    <v-layout>
                                        ...
                                        <v-flex xs12 md6>
                                            <v-text-field name="area" label="Площадь" type="text" v-model.trim="object.area" :rules="requiredRule"></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                </v-container>
                                <v-container>
                                    <v-layout>
                                        ...
                                        <v-flex xs12 md6>
                                            <v-text-field name="attendance" label="Посещаемость" type="text" v-model.trim="object.attendance" :rules="requiredRule"></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                </v-container>
                                <v-container>
                                ...
                                <v-container>
                                    ...
                                    <v-layout>
                                        <v-flex v-for="weekDay in weekDays" :key="weekDay.value">
                                            <v-checkbox
                                                :label="weekDay.name"
                                                @change="(checked) => freeDayChanged(weekDay.value)"
                                                :value="object.freeDays.includes(weekDay.value)"
                                             ></v-checkbox>
                                        </v-flex>
                                    </v-layout>
                                </v-container>
                                    ...
                                    <v-layout row justify-space-between wrap>
                                        ...
                                        <v-flex v-for="selection in selections" v-bind:key="selection.id">
                                            <v-checkbox
                                                ...
                                                :input-value="object.selections.filter(s => s.id === selection.id).length === 1"
                        ...
                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn color="primary" @click="editObjectButtonClick" :disabled="loadingInProgress">Изменить</v-btn>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async editObjectButtonClick() {
                this.object.id = this.$router.currentRoute.query.id;

                await this.axios.put('object', JSON.stringify({model: this.object}), {
                    headers: {
        ...
        async beforeMount() {
            ...
            const objectPromise = this.axios.get(`object/${this.$router.currentRoute.query.id}`);
            const selectionsPromise = this.axios.get('selections?actual=true&page=1&itemsPerPage=10000');

            const result = await Promise.all([activityTypePromise, serviceCompanyPromise, citiesPromise, objectPromise, selectionsPromise]);
            ...
            result[4].data.result.forEach(el => {
                this.selections.push({...el, isVisible: true });
            });

            this.object = result[3].data;
        }
    }
</script>
