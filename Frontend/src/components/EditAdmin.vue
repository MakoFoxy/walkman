                ...
                <v-flex xs12 sm12 md12>
                    <v-card class="elevation-12">
                        <v-toolbar dark color="primary">
                            <v-toolbar-title>Редактирование администратора</v-toolbar-title>
                            <v-spacer></v-spacer>
                        </v-toolbar>
                        <v-card-text>
                        ...
                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn color="primary" @click="editAdminButtonClick" :disabled="loadingInProgress">Редактировать</v-btn>
...
<script>
    export default {
        ...
        async beforeMount() {
            ...
            const managerResponse = await this.axios.get(`managers/${this.$router.currentRoute.query.id}`);
            this.manager = managerResponse.data;
        },
        methods: {
            async editAdminButtonClick() {
