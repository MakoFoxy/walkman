...
namespace Player.WebApi.Controllers.v1
{
    ...
    public class CurrentUserController : ControllerBase
    {
        ...
        //    Метод GetPermissions вызывается для получения прав доступа текущего пользователя. Это происходит по маршруту /api/v1/current-user/permissions. Возвращает список разрешений текущего пользователя. Это включает в себя данные о разрешениях, связанных с ролью пользователя. Действие: Получение списка разрешений для текущего пользователя. Описание: Получение списка разрешений для текущего пользователя.
        [Authorize]
        [HttpGet("objects")]
